...
class Lucid_Email_Encoder_Activate_Toolbox {
	...
	public function __construct( $file ) {
		...
		$this->wp_plugin_dir    = trailingslashit( dirname( LEEJL_PLUGIN_PATH ) );
		$this->toolbox_zip      = LEEJL_PLUGIN_PATH . 'lucid-toolbox/lucid-toolbox.zip';
	...
	public function activate_toolbox() {
		...
		elseif ( is_null( $activated ) ) :
			...
			$this->activation_error = __( 'Unknown error', 'leejl' );
			add_action( 'admin_notices', array( $this, 'toolbox_install_failed' ) );
		endif;
	}
	...
	public function toolbox_install_failed() {
		$notice = sprintf( __( 'Lucid Toolbox couldn\'t be activated, error message: %s', 'leejl' ), $this->activation_error );
	...
	public function toolbox_install_success() {
		printf( '<div class="updated"><p>%s</p></div>', __( 'Lucid Toolbox was activated successfully!', 'leejl' ) );
	}
}